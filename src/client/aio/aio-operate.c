...
int mobject_aio_write_op_operate(mobject_provider_handle_t mph,
                                 ...
                                 mobject_request_t*        req)
{
    ...
    if (ret != HG_SUCCESS) {
        fprintf(stderr,
                "[MOBJECT] margo_create() failed in "
                "mobject_aio_write_op_operate()\n");
        return -1;
    }
    ...
    if (ret != HG_SUCCESS) {
        fprintf(stderr,
                "[MOBJECT] margo_iforward() failed in "
                "mobject_aio_write_op_operate()\n");
        margo_destroy(h);
        return -1;
    }
...
int mobject_aio_read_op_operate(mobject_provider_handle_t mph,
